...
neon_past <- readRDS("data/noaa_past_climate_data.RDS")
summary(neon_past)
# loading in the phenology data

gcc_dat <- readRDS("data/gcc_use_ross.RDS")
# gcc_dat <- readRDS("data/gcc_all_data.RDS")
summary(gcc_dat)
...
gcc_dat2 <- merge(gcc_dat, neon_past, by=c("time", "siteID"), all.x=T)
summary(gcc_dat2)

# limiting the data to just 2021
gcc_dat_short <- gcc_dat2[gcc_dat2$time >= as.Date("2021-01-01")& gcc_dat2$time < as.Date("2022-01-01"),]
summary(gcc_dat_short)

gcc_dat_short <- gcc_dat_short[gcc_dat_short$siteID=="HARV",]
